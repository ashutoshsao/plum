...
async function main() {
  ...
  const bashFunction = {
    ...
    description: "Runs a bash command and returns its output. Use for: listing files, checking directories, running scripts.", parameters: {
      type: Type.OBJECT,
      properties: {
        command: {
          type: Type.STRING,
  ...
  async function runAgentLoop(userPrompt: string) {
    ...
    for (let i = 0; i < max_iters; i++) {
      ...
      //execute tools
      const toolResultParts = functionCalls.map(async fc => {
  ...
  runAgentLoop("can you check the agent.ts file, then write the agent loop in agent/agent.ts inside agent command so we can prompt from cli what we have created using commander");
}
